...
schema = Schema(title=TEXT(stored=True), path=ID(stored=True), content=TEXT(stored=True, analyzer = StandardAnalyzer(stoplist=None)))
if not os.path.exists("indexdir_stopwords"):
    os.mkdir("indexdir_stopwords")

ix = create_in("indexdir_stopwords", schema)
writer = ix.writer()
